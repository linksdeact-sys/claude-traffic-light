    ...
    try {
        Get-CimInstance Win32_Process -Filter "name='powershell.exe'" |
            Where-Object {
                ...
                $_.CommandLine -and
                $_.CommandLine -like '*ClaudeTrafficLight.ps1*' -and
...
function Set-LightTheme([string]$state) {
    ...
    $outer.BorderBrush = Set-Brush $accent
    $outer.Background = Set-Brush $soft
    $dot.Fill = Set-Brush $accent
    $status.Text = $text
    $sub.Text = $hint
}
    ...
    <Grid Margin="6">
        <Border x:Name="Outer" CornerRadius="20" BorderThickness="3" Background="#F7FAFF" BorderBrush="#0A84FF">
            ...
            <Grid Margin="18,13,16,13">
                ...
                <TextBlock x:Name="CloseText" Grid.Row="0" Grid.Column="2" Text="x" VerticalAlignment="Center" HorizontalAlignment="Center"
                           FontFamily="Segoe UI" FontSize="14" Foreground="#9AA3B2" Cursor="Hand" />
...
$closeText = $window.FindName("CloseText")

$screen = [System.Windows.SystemParameters]::WorkArea
$window.Left = $screen.Right - $window.Width - 18
$window.Top = $screen.Top + 90

$closeText.Add_MouseLeftButtonUp({ $window.Close() })
